...
use super::{
    drag_and_drop::{constrain_rect_to_area, draw_window_rect, TreeComponent},
    state::State,
...
impl<Tab> DockArea<'_, Tab> {
    ...
    pub fn show_inside(mut self, ui: &mut Ui, tab_viewer: &mut impl TabViewer<Tab = Tab>) {
        ...
        match (drag_data, hover_data) {
            (Some(source), Some(hover)) => {
            ...
            // `show_inside` may occupy only a narrow panel bank. Upstream
            // normally relies on a full-workspace DockArea to keep producing
            // hover data, so leaving that bank loses the drop entirely. Treat
            // the rest of the application as an explicit floating-window
            // destination instead.
            (Some(source), None) => {
                let pointer = ui.input(|input| input.pointer.hover_pos());
                let outside_dock = pointer.is_some_and(|pos| !ui.max_rect().contains(pos));
                let allowed_in_window = match source.src {
                ...
                if outside_dock && allowed_in_window {
                    let pointer = pointer.unwrap();
                    let window_rect = Rect::from_min_size(pointer, source.rect.size());
                    let window_rect =
                        constrain_rect_to_area(ui, window_rect, self.window_bounds.unwrap());
                    draw_window_rect(window_rect, ui, self.style.as_ref().unwrap());

                    if ui.input(|input| input.pointer.primary_released()) {
                        if let TreeComponent::Tab(surface, node, tab) = source.src {
                            self.dock_state.move_tab(
                                (surface, node, tab),
                                TabDestination::Window(window_rect),
                            );
                        }
                    }
                }
