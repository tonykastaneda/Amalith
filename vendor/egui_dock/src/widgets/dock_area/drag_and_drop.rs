...
impl DragDropState {
    ...
    fn window_preview_rect(&self, rect: Rect) -> Rect {
        // Amalith: the preview is the pane the user is moving, so changing
        // its dimensions mid-gesture is both misleading and visually jumpy.
        rect
    }
}
...
#[inline(always)]
pub(super) fn draw_window_rect(rect: Rect, ui: &Ui, style: &Style) {
    let painter = make_overlay_painter(ui);
    painter.rect_stroke(
        rect,
        0.0,
...
pub(super) fn constrain_rect_to_area(ui: &Ui, rect: Rect, mut bounds: Rect) -> Rect {
