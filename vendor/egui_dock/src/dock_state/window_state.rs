...
impl WindowState {
    ...
    pub(crate) fn create_window(&mut self, id: Id, bounds: Rect) -> egui::Window<'static> {
        ...
        let mut window_constructor = egui::Window::new("")
            .id(id)
            .constrain_to(bounds)
            // The attached-panel shrink was caused by the outer SidePanel,
            // not these floating surfaces. Keep detached panes conventionally
            // resizable from their edges and corners.
            .resizable(true)
