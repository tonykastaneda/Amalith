...
impl<Tab> DockState<Tab> {
    ...
    pub fn detach_tab(
        ...
        // Amalith: preserve the source leaf dimensions when undocking. The
        // upstream 0.8 multiplier made panes visibly shrink during a move.
...
mod test {
    ...
    #[test]
    fn detaching_from_main_surface_preserves_size() {
        let mut dock = DockState::new(vec!["panel"]);
        let rect = Rect::from_min_size(egui::pos2(40.0, 50.0), egui::vec2(248.0, 360.0));

        let surface = dock.detach_tab((SurfaceIndex::main(), NodeIndex::root(), TabIndex(0)), rect);

        assert_eq!(
            dock.get_window_state_mut(surface).unwrap().next_size(),
            Some(rect.size())
        );
    }
}
