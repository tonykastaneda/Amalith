...
fn metric_btn() -> f64 { crate::metrics::with(|m| m.panels_align_btn) }
fn metric_gap() -> f64 { crate::metrics::with(|m| m.panels_align_gap) }
...
fn metric_group_gap() -> f64 { crate::metrics::with(|m| m.panels_align_group_gap) }
...
fn metric_label_drop() -> f64 { crate::metrics::with(|m| m.panels_align_label_drop) } // section top → label baseline
fn metric_label_to_row() -> f64 { crate::metrics::with(|m| m.panels_align_label_to_row) } // label baseline → icon row top
fn metric_row_to_rule() -> f64 { crate::metrics::with(|m| m.panels_align_row_to_rule) } // icon row bottom → divider
fn metric_after_rule() -> f64 { crate::metrics::with(|m| m.panels_align_after_rule) } // divider → next section top
...
fn six(x0: f64, x1: f64, y: f64) -> [Rect; 6] {
    let g3 = 3.0 * metric_btn() + 2.0 * metric_gap();
    let left = group3(x0, y);
    let want = x0 + g3 + metric_group_gap();
    let max = (x1 - g3).max(x0 + g3 + ui_px(6.0));
...
fn layout(body: Rect) -> L {
    ...
    let section_label = |y: &mut f64| {
        let p = Point::new(x0, *y + metric_label_drop());
        *y += metric_label_drop() + metric_label_to_row();
        p
    };
    let section_rule = |y: &mut f64| {
        *y += metric_btn() + metric_row_to_rule();
        let r = Rect::new(x0, *y, x1, *y + ui_px(1.0));
        *y += ui_px(1.0) + metric_after_rule();
        r
    };
    ...
    let space_h = Rect::new(x0, y, x0 + metric_btn(), y + metric_btn());
    let space_v = Rect::new(x0 + metric_btn() + metric_gap(), y, x0 + 2.0 * metric_btn() + metric_gap(), y + metric_btn());
    let field_x0 = space_v.x1 + ui_px(8.0);
    let space_field = Rect::new(field_x0, y + ui_px(2.0), x1.max(field_x0 + ui_px(48.0)), y + metric_btn() - ui_px(2.0));
    let rule_c = section_rule(&mut y);

    // Align To: its own stacked section, left-aligned trio.
    let lab_to = section_label(&mut y);
    let to = group3(x0, y);
    y += metric_btn() + metric_pad();
...
pub fn paint(scene: &mut Scene, text: &mut TextContext, body: Rect, ctx: &Ctx) {
    ...
    let rr = l.space_field.to_rounded_rect(ui_px(3.0));
    scene.fill(
        Fill::NonZero,
        ID,
    ...
    if editing {
        ...
        let band = Rect::new(
            l.space_field.x0 + ui_px(6.0),
            l.space_field.y0 + ui_px(2.0),
            (l.space_field.x0 + ui_px(10.0) + w).min(l.space_field.x1 - ui_px(2.0)),
            l.space_field.y1 - ui_px(2.0),
        );
    ...
    text.draw(
        ...
        11.0,
        th.text,
        l.space_field.x0 + ui_px(8.0),
        l.space_field.center().y + ui_px(4.0),
    );
...
fn bar(scene: &mut Scene, r: Rect, ink: Color) {
    scene.fill(Fill::NonZero, ID, ink, None, &r.to_rounded_rect(ui_px(0.8)));
}
...
pub(crate) fn paint_align_icon(scene: &mut Scene, r: Rect, kind: AlignKind, ink: Color) {
    ...
    match kind {
        AlignKind::HLeft | AlignKind::HCenter | AlignKind::HRight => {
            let (w1, w2, h) = (ui_px(13.0), ui_px(8.0), ui_px(4.0));
            let y1 = r.y0 + ui_px(6.0);
            let y2 = r.y1 - ui_px(6.0) - h;
            let (x1, x2, lx) = match kind {
                AlignKind::HLeft => (r.x0 + ui_px(6.0), r.x0 + ui_px(6.0), r.x0 + ui_px(6.0)),
                AlignKind::HRight => (r.x1 - ui_px(6.0) - w1, r.x1 - ui_px(6.0) - w2, r.x1 - ui_px(6.0)),
            ...
            scene.stroke(
                ...
                ink,
                None,
                &Line::new((lx, r.y0 + ui_px(4.0)), (lx, r.y1 - ui_px(4.0))),
            );
        }
        AlignKind::VTop | AlignKind::VCenter | AlignKind::VBottom => {
            let (h1, h2, w) = (ui_px(13.0), ui_px(8.0), ui_px(4.0));
            let x1 = r.x0 + ui_px(6.0);
            let x2 = r.x1 - ui_px(6.0) - w;
            let (y1, y2, ly) = match kind {
                AlignKind::VTop => (r.y0 + ui_px(6.0), r.y0 + ui_px(6.0), r.y0 + ui_px(6.0)),
                AlignKind::VBottom => (r.y1 - ui_px(6.0) - h1, r.y1 - ui_px(6.0) - h2, r.y1 - ui_px(6.0)),
            ...
            scene.stroke(
                ...
                ink,
                None,
                &Line::new((r.x0 + ui_px(4.0), ly), (r.x1 - ui_px(4.0), ly)),
            );
        }
        _ => {}
    }
...
fn paint_space_icon(scene: &mut Scene, r: Rect, horiz: bool, ink: Color) {
    if horiz {
        let a = Rect::new(r.x0 + ui_px(8.0), r.y0 + ui_px(5.5), r.x0 + ui_px(16.0), r.y0 + ui_px(13.5));
        let b = Rect::new(r.x0 + ui_px(13.0), r.y0 + ui_px(12.5), r.x0 + ui_px(21.0), r.y0 + ui_px(20.5));
        bar(scene, a, ink);
        bar(scene, b, ink);
        let x = r.x0 + ui_px(4.5);
        let y = r.center().y;
        scene.stroke(&Stroke::new(ui_px(1.2)), ID, ink, None, &Line::new((x, y - ui_px(3.0)), (x, y + ui_px(3.0))));
        scene.stroke(&Stroke::new(ui_px(1.2)), ID, ink, None, &Line::new((x + ui_px(4.0), y - ui_px(3.0)), (x + ui_px(4.0), y + ui_px(3.0))));
        scene.stroke(&Stroke::new(ui_px(1.2)), ID, ink, None, &Line::new((x, y), (x + ui_px(4.0), y)));
    } else {
        let a = Rect::new(r.x0 + ui_px(6.0), r.y0 + ui_px(10.0), r.x0 + ui_px(12.0), r.y1 - ui_px(5.0));
        let b = Rect::new(r.x0 + ui_px(14.0), r.y0 + ui_px(10.0), r.x0 + ui_px(20.0), r.y1 - ui_px(5.0));
        bar(scene, a, ink);
        bar(scene, b, ink);
        let x = r.center().x;
        let y = r.y0 + ui_px(4.5);
        scene.stroke(&Stroke::new(ui_px(1.2)), ID, ink, None, &Line::new((x - ui_px(3.0), y), (x + ui_px(3.0), y)));
        scene.stroke(&Stroke::new(ui_px(1.2)), ID, ink, None, &Line::new((x - ui_px(3.0), y + ui_px(4.0)), (x + ui_px(3.0), y + ui_px(4.0))));
        scene.stroke(&Stroke::new(ui_px(1.2)), ID, ink, None, &Line::new((x, y), (x, y + ui_px(4.0))));
    }
}
